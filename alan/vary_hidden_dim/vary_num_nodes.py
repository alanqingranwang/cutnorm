...
def build_deep_lr_model(input_dim, output_dim, n_hidden, hidden_dim):
...
def train_model(data_container, hidden_dim, n_epochs_train=1000, n_batches=10):
    # Extract data
    ...
    # Construct model
    model = build_deep_lr_model(n_features, n_classes, n_hidden=1, hidden_dim=hidden_dim)
...
def main():
    ...
    hidden_dim_list = [1, 2, 3, 4, 5]
    models_params_list = []
    for hidden_dim in hidden_dim_list:
        print("Training model with size ", hidden_dim, "hidden layer")
        models_params = train_model(gauss_data, hidden_dim=hidden_dim)
